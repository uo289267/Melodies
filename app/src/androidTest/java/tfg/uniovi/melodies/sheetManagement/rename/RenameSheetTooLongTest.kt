...
class RenameSheetTooLongTest {
    private lateinit var device: UiDevice

    @Before
    fun setup() {
        device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation())
        TestUtils.setupTestUser("rename4")
    }

    @After
    fun cleanup() {
        TestUtils.cleanupTestUser("rename4")
    }
    ...
    fun renameSheetTooLongTest() {
        ...
        device.wait(Until.hasObject(By.textContains("Canción de Cuna")), 5000)
        ...
        inputField?.text = "012345678901234567899"
        ...
        onView(withId(R.id.input))
            .check(matches(hasErrorText("New name cannot be more than 20 chars long")))
