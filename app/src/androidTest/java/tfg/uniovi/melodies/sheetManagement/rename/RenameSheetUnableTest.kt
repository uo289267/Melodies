...
class RenameSheetUnableTest {
    ...
    fun setup() {
        ...
        TestUtils.setupTestUser("rename5")
    }

    @After
    fun cleanup() {
        TestUtils.cleanupTestUser("rename5")
    }
    ...
    @Test
    fun renameSheetUnableTest() {
        val originalSheetName = "Estrellita Dónde Estás"
        ...
        // Wait for the sheet item to appear inside RecyclerView
        device.wait(Until.hasObject(By.textContains("Estrellita Dónde Estás")), 5000)
        ...
        inputField?.text = "Canción de Cuna"
        ...
        // Wait up to 2 seconds for the EditText to have an error
        device.wait(Until.findObject(By.res("tfg.uniovi.melodies", "input")), 2000)
        onView(withId(R.id.input))
            .check(matches(hasErrorText("Sheet name already in use")))
